...
async def list_releases(
    ...
    total, items = await repo.list(release_type, market, artist_id, year_from, year_to, skip, limit)
    return Page(total=total, skip=skip, limit=limit, has_more=skip + limit < total, items=items)


@router.get("/{release_id}", response_model=ReleaseResponse)
async def get_release(release_id: int, db: AsyncSession = Depends(get_async_db)):
    release = await ReleaseRepository(db).get(release_id)
    if not release:
...
async def get_release_tracks(release_id: int, db: AsyncSession = Depends(get_async_db)):
    release = await ReleaseRepository(db).get_with_tracks(release_id, full=True)
    if not release:
        raise HTTPException(status_code=404, detail="Release not found")
...
async def get_release_tracks_summary(release_id: int, db: AsyncSession = Depends(get_async_db)):
    release = await ReleaseRepository(db).get_with_tracks(release_id, full=False)
    if not release:
        raise HTTPException(status_code=404, detail="Release not found")
    return sorted(release.tracks, key=lambda t: (t.disc_number or 1, t.track_number or 0))
