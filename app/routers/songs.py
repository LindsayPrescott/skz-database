...
async def list_songs(
    ...
    total, items = await repo.list(status, language, versions, skip, limit)
...
async def search_songs(
    ...
    repo = SongRepository(db)
    total, items = await repo.search(q, skip, limit)
    return Page(total=total, skip=skip, limit=limit, has_more=skip + limit < total, items=items)


@router.get("/{song_id}", response_model=SongWithCreditsResponse)
async def get_song(song_id: int, db: AsyncSession = Depends(get_async_db)):
    repo = SongRepository(db)
    song = await repo.get_with_credits(song_id)
    if not song:
...
async def get_song_versions(song_id: int, db: AsyncSession = Depends(get_async_db)):
    repo = SongRepository(db)
    original, versions = await repo.get_version_family(song_id)
    if not original:
        raise HTTPException(status_code=404, detail="Song not found")
